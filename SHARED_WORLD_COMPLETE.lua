...
local CoOwnerStore = DataStoreService:GetDataStore("MegaTycoonCoOwners_v51")
local PublicIslandStore = DataStoreService:GetDataStore("MegaTycoonPublicIslands_v51")
local GlobalLeaderboardStore = DataStoreService:GetDataStore("MegaTycoonGlobalLeaderboards_v51")
...
-- 💾 DATA LOADING WITH TELEPORT SUPPORT
...
function FactorySystem:CreatePlayerFactory(player)
    ...
    plot.Position = Vector3.new(math.random(-500, 500), 0, math.random(-500, 500))
    plot.Anchored = true
    ...
    spawn.Position = plot.Position + Vector3.new(0, 5, 0)
    spawn.Anchored = true
    ...
    sign.Position = plot.Position + Vector3.new(0, 8, -50)
    sign.Anchored = true
...
function FactorySystem:SpawnMachine(player, machineName, position)
    ...
    return true, "Machine spawned successfully"
end
...
local remotes = ReplicatedStorage:WaitForChild("SharedWorldRemotes")
...
local function createSharedWorldMenu()
    ...
    addCoOwnerBtn.MouseButton1Click:Connect(function()
        if coOwnerInput.Text ~= "" and coOwnerInput.Text ~= "Player Name..." then
...
remoteFolder.AddCoOwner.OnServerEvent:Connect(function(player, targetName)
    CoOwnerSystem:AddCoOwner(player, targetName)
end)
